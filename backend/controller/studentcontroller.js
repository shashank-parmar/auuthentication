...
exports.sendmail = async (req, res) => {
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      secure: false,
      auth: {
        user: 'devyn.wilkinson@ethereal.email',
        pass: 'tRgGTd1zDZ3ndfb6pK',
      },
    });
    ...
    res.status(200).json({ msg: 'Verification email sent', info });
  } catch (error) {
...
exports.login = async (req, res) => {
  try {
    ...
    if (!isPasswordCorrect) {
      return res.status(400).send("Password is incorrect. Please enter the correct password or reset it.");
    }
    const token = user.token;
    res.status(200).json({
      message: "Login successful. Username, email, and password are correct.",
      token: token
    });
  } catch (error) {
    console.error("Error in login:", error);  
    res.status(500).send("Internal server errorr5rrrr");
  }
};
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (verified) {
      ...
      const links = `http://localhost:5173/resetPass`;
...
//       const message = sendmaillink;
//       const studentDetails = await Student.findOne({
//         attributes: ["id", "firstName", "lastName", "email", "mobile","token"],
//         where: { id: newStudent.id },
//       });
//       console.log("password:", newStudent.password, "token:", newStudent.token);
//       res.status(201).send({ studentDetails, message: message , token});
//     } catch (error) {
...
exports.createStudent = [
  async (req, res) => {
    try {
      const { email, otp, firstName, lastName, password, mobile } = req.body;

      // Debug log to ensure body content is correct
      ...
      // Debug log to check OTP validation result
      console.log('Valid OTP:', validOtp);

      if (!validOtp) {
        return res.status(400).send('OTP is not valid or has expired');
      }

      // Check if email or firstName already exists
      const existingFirstName = await Student.findOne({ where: { firstName } });
      const existingEmail = await Student.findOne({ where: { email } });

      // Debug log to check existing records
      console.log('Existing First Name:', existingFirstName);
      ...
      if (firstName === lastName) {
        return res.status(400).send('First name and last name cannot be the same');
      }
      ...
      // Create JWT token
      const token = jwt.sign({ email, firstName, lastName }, 'shashank', { expiresIn: '1y' });
      ...
      // Respond with new student details
      res.status(201).send({
        studentDetails: {
          id: newStudent.id,
...
exports.passwordReset = async (req, res) => {
  try {
      ...
      if (updated > 0) {
          ...
          const token = jwt.sign({ email: email }, 'your_secret_key', { expiresIn: '1y' });

          return res.status(200).json({ message: 'Password updated successfully', token: token });
      } else {
          return res.status(404).json({ message: 'User not found' });
      }
  } catch (error) {
